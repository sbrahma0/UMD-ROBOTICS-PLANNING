...
sim_modelproperty_not_reset                        =0x0080 # Model is not reset at simulation end. This check is cleared at simulation end
...
sim_message_eventcallback_instancepass    =0x10d        # Called once every main application loop pass. auxiliaryData[0] contains event checks of events that happened since last time
...
# special handle checks
sim_handlecheck_assembly                =0x400000
sim_handlecheck_model                =0x800000
...
sim_dlgstyle_dont_center    =32# can be combined with one of above values. Only with this check can the position of the related UI be set just after dialog creation
...
sim_intparam_event_checks_read       =17 # can only be read
sim_intparam_event_checks_read_clear =18 # can only be read
...
simx_return_novalue_check        =0x000001        # input buffer doesn't contain the specified command
simx_return_timeout_check        =0x000002        # command reply not received in time for opmode_oneshot_wait operation mode
simx_return_illegal_opmode_check    =0x000004        # command doesn't support the specified operation mode
simx_return_remote_error_check    =0x000008        # command caused an error on the server side
simx_return_split_progress_check    =0x000010        # previous similar command not yet fully processed (applies to opmode_oneshot_split operation modes)
simx_return_local_error_check    =0x000020        # command caused an error on the client side
simx_return_initialize_error_check    =0x000040        # simxStart was not yet called

# Following for backward compatibility (same as above) 
simx_error_noerror                =0x000000
simx_error_novalue_check            =0x000001        # input buffer doesn't contain the specified command
simx_error_timeout_check            =0x000002        # command reply not received in time for opmode_oneshot_wait operation mode
simx_error_illegal_opmode_check    =0x000004        # command doesn't support the specified operation mode
simx_error_remote_error_check    =0x000008        # command caused an error on the server side
simx_error_split_progress_check    =0x000010        # previous similar command not yet fully processed (applies to opmode_oneshot_split operation modes)
simx_error_local_error_check        =0x000020        # command caused an error on the client side
simx_error_initialize_error_check        =0x000040        # simxStart was not yet called
