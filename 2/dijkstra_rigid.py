...
def creat_map(screen, w, h, r, bot_rad, clearence):
    ...
    for x_co in range(0, int(251 / r)):
        for y_co in range(0, int(151 / r)):
            flag2 = 0
...
def dijkstra_rigid(start_point, goal_point, obstacles, w, h, r, screen):
...
dia = float(input('Enter dia of the robot- '))
clearence = float(input('Enter clearance- '))
...
if r != 0:
    ...
    if ([int(sx / r), int(sy / r)] in obs):
        ...
        dijkstra_rigid((int(sx / r), int(sy / r)), (int(gx / r), int(gy / r)), obs,
